...
#define SEND(event_type) conn_fsm_handle_event(&(app_event_t){.type = (event_type)})
...
static void init_with_creds(void)
{
	...
	conn_fsm_init(hal_mock_get_iface(), network_mock_get_iface(), mqtt_mock_get_iface(),
		      ble_prov_mock_get_iface());
}
...
ZTEST(conn_fsm, test_wifi_connected_transitions_to_mqtt_connecting)
{
	...
	mqtt_mock_reset();
	SEND(EVT_WIFI_CONNECTED);
	zassert_true(mqtt_mock_is_connected(), "MQTT connect must be called after WiFi connects");
}
...
ZTEST(conn_fsm, test_mqtt_ready_transitions_to_online)
{
	...
	zassert_true(hal_mock_led_get(LED_STATUS), "LED1 must be steady ON when online");
}

ZTEST(conn_fsm, test_online_ble_stopped)
{
	reach_online();
	zassert_true(ble_prov_mock_stop_called(), "BLE advertising must stop when online");
}
...
ZTEST(conn_fsm, test_mqtt_disconnect_from_online_reconnects_mqtt)
{
	...
	zassert_false(conn_fsm_is_online(), "Must not be online after MQTT disconnect");
	zassert_true(mqtt_mock_is_connected(), "MQTT connect must be called to reconnect");
}
...
ZTEST(conn_fsm, test_reconnect_button_from_online_starts_provisioning)
{
	...
	zassert_false(conn_fsm_is_online(), "Must not be online after reconnect button");
	zassert_false(mqtt_mock_is_connected(), "MQTT must be disconnected before leaving ONLINE");
	zassert_true(ble_prov_mock_start_called(),
...
ZTEST(conn_fsm, test_session_events_ignored)
{
	...
	zassert_false(conn_fsm_is_online(), "Session events must not change connectivity state");
}
